...
Referências para estudar detecção, descritores, rastreamento e anotação de resultados. Os exemplos executáveis usam bibliotecas Python locais; não exigem conta Roboflow ou chave de API. Fontes consultadas em **26 de setembro de 2026**.

As páginas `latest` e o blog podem mudar após a consulta. Por isso, a reprodução depende das versões fixadas e dos arquivos verificados, não apenas da data de um tutorial.
...
As APIs utilizadas correspondem a `supervision==0.30.5`, `trackers==2.6.1` e `rfdetr==1.11.0`. Tutoriais de versões anteriores podem apresentar outros métodos e valores padrão.
...
- O download é feito diretamente da fonte pública original pelas células de preparação.
